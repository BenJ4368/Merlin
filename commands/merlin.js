		...
		async execute(interaction) {
			...
			try {
				...
				if (!searchResponse.data.data || searchResponse.data.data.length === 0) {
					await interaction.reply({ content: `Aucun résultat trouvé pour **${titre}**.`, ephemeral: true });
					return;
				}
				...
				await interaction.reply({
					content: `Une erreur s'est produite lors de la récupération des informations pour **${titre}**.`,
					ephemeral: true
				});
			}
		}
